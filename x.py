...
def main():
    ...
    directory = "H=x_dH=50_da=0.05_shape"   # 結果を保存するディレクトリ名
    os.mkdir(directory)  # ディレクトリを作成

    # シミュレーション設定
    t_limit = 1e-9  # シミュレーションの終了時間 [秒]
    dt = t_limit / 1e3  # タイムステップ [秒]
    limit = int(t_limit / dt)  # ステップ数
    alphaG = 0.01  # ギルバート減衰定数
    anisotropy = np.array([0e0, 0e0, 0e0])  # 異方性定数 [Oe]
    H_shape = np.array([0.012, 0.98, 0.008]) * 18850  # 反磁場 [Oe]
    dh = 50  # １回の行動での磁場変化 [Oe]
    da = 5e-11  # 行動間隔 [秒]  da<=1e-10
