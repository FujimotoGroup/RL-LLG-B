...
def main():
    ...
    for episode in range(episodes):
        ...
        for i in np.arange(dynamics.limit):
            if i % 400 == 0:
                action = agent.get_action(dynamics.m)
                h0 = action - 1
                if i == 0:
                    old_action = action
            ...
            if i % 400 == 0:
                ...
                if i != 0:
#                    agent.update(old_m, old_action, old_reward, dynamics.m, done)
                    loss = agent.update(old_m, old_action, old_reward, dynamics.m, done)
                    if episode > 20:
                        total_loss += loss
                        cnt += 1
        ...
        if total_reward > high_reward:
            s.save_episode(episode, t, m, h, "fig_dqn")
            high_reward = total_reward

#        s.save_prob(episode, agent, "fig_dqn")
        ...
        if episode > 20:
            ...
            s.save_loss_history(loss_history, "fig_dqn")

        s.save_reward_history(reward_history, "fig_dqn")
