...
def main():
    ...
    for episode in range(episodes):
        ...
        for i in np.arange(dynamics.limit):
            if i % 400 == 0:
                action = agent.get_action(dynamics.m, epsilon)
                h0 = action - 1
                if i == 0:
                    old_action = action
...
#                    agent.update(old_m, old_action, old_reward, dynamics.m, done)
        ...
        if total_reward > high_reward:
            s.save_episode(episode, t, m, h, "fig_dqn2")
        ...
        if loss is not None:
            ...
            s.save_loss_history(loss_history, "fig_dqn2")

        s.save_reward_history(reward_history, "fig_dqn2")
